...
if __name__ == "__main__":
    ...
    for data_type in data_type_list:
        ...
        for fp_llm in fname_llm:
            ...
            traj_goal = np.genfromtxt(fp_traj2, delimiter=',').transpose()  #traj_goal[0:2,:]:trajectory (pixel), traj_goal[2:4,:]:goalpos (pixel)
            ...
            d = np.sqrt(np.sum((traj_goal[0:2,:] - traj_goal[2:4,:])**2, axis=0))/128*10
            ...
            dpos_ = np.sqrt(np.sum((traj_goal[0:2,1:] - traj_goal[0:2,:-1])**2, axis=0))/128*10
            dpos = np.zeros(traj_goal.shape[1])
            ...
            acc_top1_llm.append(np.sum(gt_idx==order_llm[0]))
            acc_top3_llm.append(np.sum(gt_idx==order_llm[0:3]))
            acc_top5_llm.append(np.sum(gt_idx==order_llm[0:5]))
            
            
            for i in range(order_traj.shape[1]):
                
                if i >= d1: 
                    acc_top1_traj[0].append(np.any(np.in1d(gt_idx2, order_traj[0,i])))
                    acc_top3_traj[0].append(np.any(np.in1d(gt_idx2, order_traj[0:3,i])))
                    acc_top5_traj[0].append(np.any(np.in1d(gt_idx2, order_traj[0:5,i])))
                    
                    acc_top1_ours[0].append(np.any(np.in1d(gt_idx2, order_ours[0,i])))
                    acc_top3_ours[0].append(np.any(np.in1d(gt_idx2, order_ours[0:3,i])))
                    acc_top5_ours[0].append(np.any(np.in1d(gt_idx2, order_ours[0:5,i])))
                
                if i >= d2:
                    acc_top1_traj[1].append(np.any(np.in1d(gt_idx2, order_traj[0,i])))
                    acc_top3_traj[1].append(np.any(np.in1d(gt_idx2, order_traj[0:3,i])))
                    acc_top5_traj[1].append(np.any(np.in1d(gt_idx2, order_traj[0:5,i])))
                    
                    acc_top1_ours[1].append(np.any(np.in1d(gt_idx2, order_ours[0,i])))
                    acc_top3_ours[1].append(np.any(np.in1d(gt_idx2, order_ours[0:3,i])))
                    acc_top5_ours[1].append(np.any(np.in1d(gt_idx2, order_ours[0:5,i])))
            
                if i >= d3:
                    acc_top1_traj[2].append(np.any(np.in1d(gt_idx2, order_traj[0,i])))
                    acc_top3_traj[2].append(np.any(np.in1d(gt_idx2, order_traj[0:3,i])))
                    acc_top5_traj[2].append(np.any(np.in1d(gt_idx2, order_traj[0:5,i])))
                    
                    acc_top1_ours[2].append(np.any(np.in1d(gt_idx2, order_ours[0,i])))
                    acc_top3_ours[2].append(np.any(np.in1d(gt_idx2, order_ours[0:3,i])))
                    acc_top5_ours[2].append(np.any(np.in1d(gt_idx2, order_ours[0:5,i])))
        
        out_acc_pct_llm1 = np.zeros([1,1])
        out_acc_pct_llm3 = np.zeros([1,1])
        out_acc_pct_llm5 = np.zeros([1,1])
        out_acc_pct_traj1 = np.zeros([3,1])
        out_acc_pct_traj3 = np.zeros([3,1])
        out_acc_pct_traj5 = np.zeros([3,1])
        out_acc_pct_ours1 = np.zeros([3,1])
        out_acc_pct_ours3 = np.zeros([3,1])
        out_acc_pct_ours5 = np.zeros([3,1])
        ...
        out_acc_pct_llm1[0,0] = np.mean(acc_top1_llm)*100
        out_acc_pct_llm3[0,0] = np.mean(acc_top3_llm)*100
        out_acc_pct_llm5[0,0] = np.mean(acc_top5_llm)*100

        for j in range(3):
            out_acc_pct_traj1[j,0] = np.mean(acc_top1_traj[j])*100
            out_acc_pct_traj3[j,0] = np.mean(acc_top3_traj[j])*100
            out_acc_pct_traj5[j,0] = np.mean(acc_top5_traj[j])*100
            out_acc_pct_ours1[j,0] = np.mean(acc_top1_ours[j])*100
            out_acc_pct_ours3[j,0] = np.mean(acc_top3_ours[j])*100
            out_acc_pct_ours5[j,0] = np.mean(acc_top5_ours[j])*100
        ...
        header = np.array(["llm_top1","llm_top3","llm_top5","traj_top1(d>1m)","traj_top1(d>2m)","traj_top1(d>3m)","traj_top3(d>1m)","traj_top3(d>2m)","traj_top3(d>3m)","traj_top5(d>1m)","traj_top5(d>2m)","traj_top5(d>3m)","llm+traj_top1(d>1m)","llm+traj_top1(d>2m)","llm+traj_top1(d>3m)","llm+traj_top3(d>1m)","llm+traj_top3(d>2m)","llm+traj_top3(d>3m)","llm+traj_top5(d>1m)","llm+traj_top5(d>2m)","llm+traj_top5(d>3m)"])
        out_pct = np.concatenate([header.reshape(1,-1),out_pct],axis=0)
        
        np.savetxt(folder_path_eval + "eval.csv", out_pct, delimiter=',', fmt='%s')