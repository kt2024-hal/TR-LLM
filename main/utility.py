...
"""
def get_path_map(gt_future, scene_image, n_len):
	...
	for n in n_len:
		...
		gt_path_map = []
		img_tmp = cv2.resize(scene_image, (256,256), interpolation=cv2.INTER_NEAREST).reshape(256,256,1)*255
		img = np.concatenate([img_tmp,img_tmp,img_tmp],axis=2)
		x2 = 0
		y2 = 0
		...
		for k in range(n):
			x1 = x2
			y1 = y2
			x2 = int(gt_future[k,0])
			y2 = int(gt_future[k,1])
            
			if k>0:
				cv2.line(img, (x1, y1), (x2, y2), (0,0,255), thickness=2)
			if k==0:
				cv2.circle(img, (x2, y2), 5, (0,0,255), thickness=-1)
    
	#cv2.imwrite("./a.png",img)
	return img
"""
def get_path_map(traj, img_in):
        
    img = np.concatenate([img_in,img_in,img_in],axis=2)
    x2 = 0
    y2 = 0
...
def get_goal_area(map_binary, traj_map, map_size):
    ...
    model.load_state_dict(torch.load('./model_pred_goal/model_goal.pt')['model_state_dict'])
