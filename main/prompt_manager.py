...
def aconv_image(img_array):

    #img_array = cv2.resize(np.array(img_array), (512, 512), interpolation=cv2.INTER_NEAREST)
    
    #img_path = "./tmp_img.jpg"
...
def proc_chatgpt(assistant_pt, order_pt, hidden_pt, res_type, past_pt=[], seed=st.seed):
    ...
    client = OpenAI(api_key=st.API_KEY)
...
def get_order_get_target_object2(name, obj_pt, pred_pt):
    ...
    order_pt += ("- Make sure that the objects must exactly the same as that in the [OBJECT LIST].\n")
